            ...
            <View className="flex-row items-center gap-3 py-0.5">
              {props.state?.checked && (
                <Animated.View entering={FadeInLeft} exiting={FadeOutLeft}>
                  <Icon name="checkmark" className="text-foreground size-5" />
                </Animated.View>
              )}
              ...
              {props.state && !props.state?.checked && (
                <Icon name="checkmark" className="text-foreground/0 size-5" />
              )}
            </View>
          )}
          {props.image ? (
...
function DropdownMenuSubMenu({ title, subTitle, items }: Omit<DropdownSubMenu, 'loading'>) {
  ...
  function addSubMenuRef(ref?: DropdownMenuMethods | null) {
    if (!ref) return;
