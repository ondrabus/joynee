...
import type { ContextItem, ContextMenuProps, ContextMenuMethods, ContextSubMenu } from './types';
            ...
            <View className="flex-row items-center gap-3 py-0.5">
              {props.state?.checked && (
                <Animated.View entering={FadeInLeft} exiting={FadeOutLeft}>
                  <Icon name="checkmark" size={21} color={colors.foreground} />
                </Animated.View>
              )}
              ...
              {props.state && !props.state?.checked && (
                <Icon name="checkmark" size={21} color="#00000000" />
              )}
            </View>
          )}
          {props.image ? (
...
function ContextMenuSubMenu({ title, subTitle, items }: Omit<ContextSubMenu, 'loading'>) {
  ...
  function addSubMenuRef(ref: ContextMenuMethods | null) {
    if (!ref) return;
